...
OUTPUT_PATH = Path("output/player_match_fantasy_v4.csv")
...
def add_fantasy_points_v4(df):
    ...
    df["fantasy_points_v4"] = df[score_cols].sum(axis=1)
    return df

def main():
    df = pd.read_csv(INPUT_PATH, low_memory=False)
    validate_columns(df)
    out = add_fantasy_points_v4(df)
    out.to_csv(OUTPUT_PATH, index=False)

    print("Input rows:", len(df))
    print("Saved:", OUTPUT_PATH)
    print("\nFantasy points v4 summary:")
    print(out["fantasy_points_v4"].describe())
    print("\nTop 15 rows:")
    print(
        out.sort_values("fantasy_points_v4", ascending=False)[
            ["match_id", "player_name", "team", "player_role_platform", "runs_scored", "wickets", "maidens", "runout_direct", "runout_assist", "fantasy_points_v4"]
